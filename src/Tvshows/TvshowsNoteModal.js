...
export default function
  ...
  const handleOpen = () => {
   ...
   let tvshowsDocWatched = db.collection("users")
      .doc(user.email)
      .collection("tvshows")
      .doc("section")
      .collection("watched")
   ...
   let tvshowsDocToWatch = db.collection("users")
      .doc(user.email)
      .collection("tvshows")
      .doc("section")
      .collection("to-watch")
      ...
      .then(docWatched => {
        ...
        if (docWatched.exists) {
          ...
          if (docWatched.data().note) {
           ...
           tvshowsDocToWatch.get()
            .then(doc => {
              // check if show doc exists in to-watch list if it does add note
              if (doc.exists) {
               tvshowsDocToWatch.update({ note: docWatched.data().note })
              }
            })
          }
                ...
                if (docToWatch.data().note) {
                 ...
                 tvshowsDocWatched.get()
                    .then(doc => {
                      ...
                      if (doc.exists) {
                       tvshowsDocWatched.update({ note: docToWatch.data().note })
                      }
                    })
              }
            }
  ...
  const handleSave = () => {
   ...
   tvshowsDocWatched.get()
      .then(doc => {
        // check if show doc exists in list 
        if (doc.exists) {
         tvshowsDocWatched.update({ note: value })
        }
       ...
       tvshowsDocToWatch.get()
          .then(doc1 => {
            // check if show doc exists in list 
            if (doc1.exists) {
             tvshowsDocToWatch.update({ note: value })
            }
          })
      })
    handleClose()
  ...
  const handleDelete = () => {
   ...
   tvshowsDocWatched.get()
      .then(doc => {
        // check if show doc exists in list 
        if (doc.exists) {
         tvshowsDocWatched.update({ note: firebase.firestore.FieldValue.delete() })
   ...
   tvshowsDocToWatch.get()
      .then(doc => {
        // check if show doc exists in list 
        if (doc.exists) {
         tvshowsDocToWatch.update({ note: firebase.firestore.FieldValue.delete() })
